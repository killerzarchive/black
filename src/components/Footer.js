...
const cols = [
  { heading: 'Products', links: ['BlackFlow', 'VaultOS', 'DataPulse', 'EdgeNet', 'DeployKit', 'NeuralBase'] },
  { heading: 'Companies', links: ['Obsidian Labs', 'NovaSoft', 'PulseMedia', 'ChainVault', 'AxisCloud', 'StudioAI'] },
...
export default function Footer() {
  return (
    <footer style={{ borderTop: '1px solid rgba(0,0,0,0.06)', padding: '80px 40px 48px', background: '#fff' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '240px repeat(4, 1fr)', gap: '40px', marginBottom: '64px' }} className="footer-grid">
          <div>
          ...
          {cols.map(col => (
            <div key={col.heading}>
              <h5 style={{ fontSize: '12px', fontWeight: 600, color: 'rgba(0,0,0,0.35)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '20px' }}>
                {col.heading}
              </h5>
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {col.links.map(link => (
                  <li key={link}>
                    <a
                      href="#"
                      style={{ fontSize: '14px', color: 'rgba(0,0,0,0.45)', textDecoration: 'none', transition: 'color 0.2s' }}
        ...
        <div style={{ borderTop: '1px solid rgba(0,0,0,0.06)', paddingTop: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '24px' }}>
            {['Twitter', 'LinkedIn', 'GitHub'].map(s => (
              <a
                key={s} href="#"
                style={{ fontSize: '13px', color: 'rgba(0,0,0,0.3)', textDecoration: 'none', transition: 'color 0.2s' }}
